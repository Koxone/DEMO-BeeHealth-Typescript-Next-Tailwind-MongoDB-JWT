...
function DateInput({ question, required, value, id, onChange }) {
  ...
  const updateDate = (field, newValue) => {
    ...
    if (newParsed.year && newParsed.month && newParsed.day) {
      ...
      onChange(formatted);
    } else if (newParsed.year || newParsed.month || newParsed.day) {
      // Fecha parcial
      const y = newParsed.year || '';
      const m = newParsed.month ? String(newParsed.month).padStart(2, '0') : '';
      const d = newParsed.day ? String(newParsed.day).padStart(2, '0') : '';
      onChange(`${y}-${m}-${d}`);
    } else {
      onChange('');
    }
