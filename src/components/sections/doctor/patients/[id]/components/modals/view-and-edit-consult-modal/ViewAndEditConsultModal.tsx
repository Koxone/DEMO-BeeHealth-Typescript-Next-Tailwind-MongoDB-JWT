...
export default function ViewAndEditConsultModal({
  onClose,
  record,
  specialty,
  readOnly,
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.ok) {
      ...
      // Snapshot update
      setNewCurrentWeight(Number(formData[7]));
      setNewCurrentSize(Number(formData[8]));

      // Edit user current weight and size with editUser Custom Hook
      await editUser({
        userId: patientId,
        currentWeight: Number(formData[7]),
        currentSize: Number(formData[8]),
      });
