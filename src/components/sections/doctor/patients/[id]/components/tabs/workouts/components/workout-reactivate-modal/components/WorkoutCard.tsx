...
function WorkoutCard({ selectedWorkout, isDeactivating, formatDate }) {
  return (
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-lg font-bold text-gray-900">{selectedWorkout?.workout?.name}</p>
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              >
                {selectedWorkout?.isActive ? 'Activa' : 'Inactiva'}
              </span>
            </div>
            <p className="mt-2 text-xs text-gray-500">
              Asignada: {formatDate(selectedWorkout?.assignedAt)}
            </p>
          </div>
        </div>
      </div>
