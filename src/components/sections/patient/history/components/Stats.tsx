'use client';
import { Weight, TrendingDown, TrendingUp, Award, Ruler } from 'lucide-react';
    ...
    <div className="bg-beehealth-body-main rounded-2xl border-gray-200 shadow-sm transition-all duration-300 hover:shadow-lg md:border-2 md:p-6">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <StatCard
          ...
          value={
            type === 'size'
              ? patientWeightLogs[0]?.originalSize?.toFixed(1)
              : patientWeightLogs[0]?.originalWeight?.toFixed(1)
          }
          unit={labels.unit}
        />
        ...
        <StatCard
          ...
          value={
            type === 'size'
              ? patientWeightLogs[0]?.currentSize?.toFixed(1)
              : patientWeightLogs[0]?.currentWeight?.toFixed(1)
          }
          unit={labels.unit}
        />
        ...
        <StatCard
          ...
          value={
            type === 'size'
              ? (patientWeightLogs[0]?.originalSize - patientWeightLogs[0]?.currentSize)?.toFixed(1)
              : (
