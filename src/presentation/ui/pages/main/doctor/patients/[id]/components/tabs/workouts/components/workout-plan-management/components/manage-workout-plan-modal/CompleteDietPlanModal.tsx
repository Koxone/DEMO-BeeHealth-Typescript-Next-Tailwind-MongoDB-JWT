...
// Prop Types
interface CompleteDietPlanModalProps {
  selectedDiet: DietPlanResponseDTOPresentation;
}

export default function CompleteDietPlanModal({ selectedDiet }: CompleteDietPlanModalProps) {
  ...
  const { mutate: completeDietPlan, isPending } = useCompleteDietPlan();

  const isFormValid = complianceStatus && rating > 0 && doctorNotes.trim().length > 0;

  const handleAssign = async () => {
    if (!selectedDiet || !isFormValid) return;
    ...
    completeDietPlan(
      ...
      },
      {
        onSuccess: () => {
          closeModal();
          openModal('success', { title: 'Éxito', message: 'La dieta se completó correctamente.' });
          setTimeout(() => {
            closeModal();
          }, 1200);
        },
        onError: (err: any) => {
          console.error('Error al completar la dieta:', err);
        },
      }
    );
  };
      ...
      <div className="relative inset-0 z-50 flex w-full max-w-xl items-center justify-center p-4">
        ...
        >
          ...
          <div className="relative max-h-[70vh] overflow-y-auto p-6">
            <DietCard selectedDiet={selectedDiet} isDeactivating={false} />
