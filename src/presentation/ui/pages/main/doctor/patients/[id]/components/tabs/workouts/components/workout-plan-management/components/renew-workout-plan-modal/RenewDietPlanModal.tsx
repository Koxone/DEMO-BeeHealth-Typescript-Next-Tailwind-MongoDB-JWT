...
import DoctorNotes from '../manage-workout-plan-modal/components/DoctorNotes';
...
// Prop Types
interface RenewDietPlanModalProps {
  consultationId: string;
  selectedDiet: DietPlanResponseDTOPresentation;
}

export default function RenewDietPlanModal({
  selectedDiet,
  consultationId,
}: RenewDietPlanModalProps) {
  ...
  const { mutate: renewDietPlan } = useRenewDietPlan();
  ...
  const handleRenew = async () => {
    ...
    renewDietPlan(
      ...
      },
      {
        onSuccess: () => {
          closeModal();
          openModal('success', { title: 'Éxito', message: 'La dieta se renovó correctamente.' });
          setTimeout(() => {
            closeModal();
          }, 1200);
        },
        onError: (err: any) => {
          console.error('Error al renovar la dieta:', err);
        },
      }
    );
  };
